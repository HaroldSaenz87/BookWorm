...
export default function Profile() {
  ...
  const [books, setBooks] = useState<{ _id: string }[]>([]);
  ...
  const fetchData = async() => {
    ...
    try{
      ...
      const response = await fetch("https://bookworm-xkkg.onrender.com/api/books/user",{
  ...
  const handleDeleteBook = async(bookId: string) => {
    ...
    try {
      ...
      const response = await fetch(`https://bookworm-xkkg.onrender.com/api/books/${bookId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}`},
      })
      ...
      if(!response.ok){
        ...
        throw Error(data.message || "Failed to delete book");
  ...
  const renderBookItem = ({item}: {item : any}) => {
    ...
    return(
      ...
      <View style={styles.bookItem}>
        ...
        <TouchableOpacity style={styles.deleteButton} onPress={() => confirmDel(item._id)}>

          {deletedBook === item._id ? (
